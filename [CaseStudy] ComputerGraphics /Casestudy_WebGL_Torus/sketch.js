...
function draw() {
    ...
    for(var v = 0; v < slice-1; v++){
        for(var u = 0; u < segment; u++){
            ...
            var phi = 2 * pi * v/(slice-1);
            var radius = 0.4;
            var scale = 0.6;

            var px = (1 + radius * Math.cos(phi)) * Math.cos(theta) * scale;
            var py = (1 + radius * Math.cos(phi)) * Math.sin(theta) * scale;
            var pz = radius * Math.sin(phi) * scale;
            ...
            phi = 2 * pi * (v+1)/(slice-1);

            px = (1 + radius * Math.cos(phi)) * Math.cos(theta) * scale;
            py = (1 + radius * Math.cos(phi)) * Math.sin(theta) * scale;
            pz = radius * Math.sin(phi) * scale;
