...
HSpline.prototype.transform = function(_g, _m){
    ...
    ////draw curve//--->
    var tRatio = 20; //<-smoothness of curve
...
HSpline.prototype.addPoint = function(_x, _y){
    var pos = new Vector3(0, 0, 0);
    pos = modelCoord(_x, _y);
    this.positions.push( pos ); 
};
HSpline.prototype.resetPoints = function(){
    this.positions = [];
};
HSpline.prototype.detectPoints = function(_mousePos){
    var size = this.positions.length;
    var offset = 30;
...
HSpline.prototype.movePoints = function(_index, _mousePos){
    var i = _index;
    var m = _mousePos;
    var pos = modelCoord(m.x, m.y);
    this.positions[i].x = pos.x;
    this.positions[i].y = pos.y;
};
HSpline.prototype.deletePoints = function(_index){
    var i = _index;
...
BSpline.prototype.transform = function(_g, _m){
    ...
    ////draw curve//--->
    var tRatio = size * 10; //<-smoothness of curve
    context.beginPath();
    if(size > 0){
        var root = pixelCoord(this.positions[0].x, this.positions[0].y);
        context.moveTo(root.x, root.y);
...
BSpline.prototype.addPoint = function(_x, _y){
    var pos = new Vector3(0, 0, 0);
    pos = modelCoord(_x, _y);
    this.positions.push( pos ); 
};
BSpline.prototype.resetPoints = function(){
    this.positions = [];
};
BSpline.prototype.detectPoints = function(_mousePos){
    var size = this.positions.length;
    var offset = 30;
...
BSpline.prototype.movePoints = function(_index, _mousePos){
    var i = _index;
    var m = _mousePos;
    var pos = modelCoord(m.x, m.y);
    this.positions[i].x = pos.x;
...
BSpline.prototype.deletePoints = function(_index){
    var i = _index;
    this.positions.splice(i, 1);
};
